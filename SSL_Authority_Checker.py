# SSL Autohrity Checker - 23/05/2025.
# Author @ Cam Noakes

# See 'readme.md' file for specific details.
...
# Load environment variables from the .env file (not included for obvious reasons).
load_dotenv()

# Email configuration for SMTP Usage.
...
# Allow list of trusted authorities for a corporation.
TRUSTED_AUTHORITIES = [
    "Let's Encrypt",
    "DigiCert",
    "WR2",
...
# Read the hosts from the hosts.csv file located in the same directory.
        ...
        with open(file_path, newline='') as csvfile:
            ...
            for row in reader:
                if row:
                    ...
                    # once each host has been selected and formatted (newline stripped) add it to the array 'hosts'
                    hosts.append(host)
...
# Over port 443 (HTTPS) obtain the SSL certificate chain. Internet connection is required for this, ensure you have it.
def get_certificate_chain(host, port=443):
    print("Connecting to host: " + host) # Logging purposes if running as a cron job.
    
    # Attempt to obtain the SSL cert.
    try:
...
# Extracts key ssl cert info from eachchain.

# This is the main part,  the SSL certificate chain is retrieved from the host. It
# extracts the issuer's name from the first certificate, gathers the serial numbers
# from all certificates in the chain, then checks whether the issuer is considered
# a trusted authority by comparing it against the allowlist.
...
# if the issuer name is in trusted array return true.
...
# Save serials to json for easy retrieval.
...
# Email notification alert sending...
...
# Configuring the logging and timestamped .log files.
def setup_logging():
